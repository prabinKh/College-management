...
def forgot_password_view(request):
    if request.method == 'POST':
        ...
        if user:
            if user.is_student:
                messages.error(request, 'Students cannot reset passwords. Please contact an admin.')
                return redirect('forgot-password')  # Redirect instead of render
        ...
        return redirect('forgot-password')  # Always redirect after POST
...
def logout_view(request):
    logout(request)
    messages.success(request, 'You have been logged out.', extra_tags='success')  # Ensure message is tagged
    return redirect('index')